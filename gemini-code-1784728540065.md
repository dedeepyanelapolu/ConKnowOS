...
ContextOS sits between user applications and LLM providers. It uses a **Go Microkernel**, **Model Context Protocol (MCP)** servers, and an **Agent-to-Agent (A2A)** event bus to handle memory retrieval, prompt compression, state checkpointing, and tool execution prior to calling LLMs.
...
| :--- | :--- | :--- |
| **Core Microkernel** | **Go (Golang 1.22+)** | Concurrency engine, Workflow Runner, state machine, HTTP/gRPC gateway |
...
| **Observability Dashboard**| **React + React Flow** | Live execution graph rendering, step latency, and token cost metrics |
...
├── kernel/                         <-- Go Microkernel Core
│   ├── go.mod
│   ├── cmd/
│   │   └── main.go                 <-- Entrypoint & Health API Gateway
│   ├── config/
│   │   └── config.go               <-- Pydantic / Environment loader
│   └── internal/
│       ├── state/                  <-- Workflow Runner & Postgres Checkpointer
...
├── frontend/                       <-- React + React Flow Telemetry Portal
├── sdk/                            <-- Go & Python Client SDKs
